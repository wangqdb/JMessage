...
JPushIM = (function() {
	  ...
	  JPushIM.init = function(options) {
		  ...
		  this.socket.on('loginEventGetUID', function(data){
			  options.onLoginGetUID(data);
		  });
	   ...
	   JPushIM.bindSocketIoClientEvent = function(options) {
	   ...
	   JPushIM.getGroupMemberListEvent = function(options){
	   ...
	   JPushIM.disconnectEvent = function(options) {
		...
		JPushIM.emoji = {};
		JPushIM.emoji.tabOne = new Array(':bowtie:',':smile:',':laughing:',':blush:',':smiley:',':relaxed:',
		...
		JPushIM.emoji.tabTwo = new Array(':arrow_double_down:', ':arrow_double_up:', ':arrow_down_small:', ':arrow_heading_down:', 
		...
		JPushIM.initEmojiPanelOne = function(divId){
			//设置基本表情的面板数据
			for (var i=0; i<JPushIM.emoji.tabOne.length; i++) {  
				var emoji = JPushIM.emoji.tabOne;
				$("<li onclick='selectEmotionImg(this);' id="+emoji[i]+">"+emoji[i]+"</li>").appendTo($(divId));
			};
			emojify.run();
		};
		JPushIM.initEmojiPanelTwo = function(divId){
			//  设置动画表情的面板数据
			for (var i=0; i<JPushIM.emoji.tabTwo.length; i++) {  
				var emoji = JPushIM.emoji.tabTwo;
				$("<li onclick='selectEmotionImg(this);' id="+emoji[i]+">"+emoji[i]+"</li>").appendTo($(divId));
			};
			emojify.run();
		};
